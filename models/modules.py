...
def get_embed(inputs, num_inputs, embed_size, name):
  embed_table = tf.get_variable(
    name, [num_inputs, embed_size], dtype=tf.float32, initializer=tf.truncated_normal_initializer(stddev=0.1)
  )
  ...
  with tf.variable_scope(scope):
    with tf.variable_scope('conv_bank'):
      ...
      conv_outputs = tf.concat(
        [conv_fn(k) for k in range(1, bank_size + 1)],
        axis=-1,
      )
...
def highwaynet(inputs, scope):
  ...
  with tf.variable_scope(scope):
    ...
    T = tf.layers.dense(
      inputs, units=highway_dim, activation=tf.nn.sigmoid, name='T', bias_initializer=tf.constant_initializer(-1.0)
    )
