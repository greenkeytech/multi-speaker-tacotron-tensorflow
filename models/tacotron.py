  ...
  def initialize(
    ...
    with tf.variable_scope('inference') as scope:
      ...
      if self.num_speakers > 1:
        if hp.speaker_embedding_size != 1:
          speaker_embed_table = tf.get_variable(
            'speaker_embedding', [self.num_speakers, hp.speaker_embedding_size],
            dtype=tf.float32,
            initializer=tf.truncated_normal_initializer(stddev=0.5)
          )
        ...
        if hp.model_type == 'deepvoice':
          if hp.speaker_embedding_size == 1:
            ...
            encoder_rnn_init_state = get_embed(
              speaker_id, self.num_speakers, hp.enc_rnn_size * 2, "encoder_rnn_init_state"
            )

            attention_rnn_init_state = get_embed(
              speaker_id, self.num_speakers, hp.attention_state_size, "attention_rnn_init_state"
            )
      ...
      dec_prenet_outputs = DecoderPrenetWrapper(
        GRUCell(hp.attention_state_size), speaker_embed, is_training, hp.dec_prenet_sizes, hp.dropout_prob
      )
