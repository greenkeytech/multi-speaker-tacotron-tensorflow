...
class Synthesizer(object):
  ...
  def load(self, checkpoint_path, num_speakers=2, checkpoint_step=None, model_name='tacotron'):
    ...
    speaker_id = tf.placeholder_with_default(tf.zeros([batch_size], dtype=tf.int32), [None], 'speaker_id')

    load_hparams(hparams, load_path)
    with tf.variable_scope('model') as scope:
      self.model = create_model(hparams)

      self.model.initialize(inputs, input_lengths, self.num_speakers, speaker_id)
      self.wav_output = \
    ...
    sess_config = tf.ConfigProto(
      allow_soft_placement=True, intra_op_parallelism_threads=1, inter_op_parallelism_threads=2
    )
...
def plot_graph_and_save_audio(
  ...
  if use_short_concat:
    wav = short_concat(
      wav, alignment, text, start_of_sentence, end_of_sentence, pre_word_num, post_word_num, pre_surplus_idx,
      post_surplus_idx
    )
