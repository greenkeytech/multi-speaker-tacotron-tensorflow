    ...
    with tf.variable_scope('gap_test-train') as scope:
      summaries.extend(
        [
          ...
          tf.summary.scalar('loss', model.loss_without_coeff - model2.loss_without_coeff),
        ]
      )
...
def save_and_plot_fn(args, log_dir, step, loss, prefix):
  ...
  if 'korean_cleaners' in [x.strip() for x in hparams.cleaners.split(',')]:
    ...
    plot.plot_alignment(
      ...
      align_path,
      info=info_text,
      text=sequence_to_text(seq, skip_eos_and_pad=True, combine_jamo=True),
      isKorean=True
    )
  else:
    ...
    plot.plot_alignment(
      ...
      align_path,
      info=info_text,
      text=sequence_to_text(seq, skip_eos_and_pad=True, combine_jamo=False),
      isKorean=False
    )
...
def train(log_dir, config):
  ...
  with tf.variable_scope('datafeeder') as scope:
    train_feeder = DataFeeder(coord, data_dirs, hparams, config, 32, data_type='train', batch_size=hparams.batch_size)
    test_feeder = DataFeeder(coord, data_dirs, hparams, config, 8, data_type='test', batch_size=config.num_test)
    ...
    try:
      ...
      while not coord.should_stop():
        ...
        step, loss, opt = sess.run(
          [global_step, model.loss_without_coeff, model.optimize], feed_dict=model.get_dummy_feed_dict()
        )
        ...
        log(message, slack=(step % (10 * config.checkpoint_interval) == 0))
        ...
        if step % config.test_interval == 0:
          ...
          save_and_plot(sequences[:1], spectrograms[:1], alignments[:1], log_dir, step, loss, "train")
...
def main():
  ...
  parser.add_argument(
    ...
    default="https://hooks.slack.com/services/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"
  )
