...
def text_recognition(path, config):
  ...
  while True:
    try:
      ...
      content = load_audio(
        path, pre_silence_length=config.pre_silence_length, post_silence_length=config.post_silence_length
      )
...
def text_recognition_batch(paths, config):
  ...
  results = {}
  items = parallel_run(partial(text_recognition, config=config), paths, desc="text_recognition_batch", parallel=True)
  for item in items:
    results.update(item)
  return results
