...
def amplify(path, keep_silence=300):
  ...
  nonsilent_ranges = pydub.silence.detect_nonsilent(sound, silence_thresh=-50, min_silence_len=300)
...
def generate_audio_response(text, speaker_id):
  ...
  relative_audio_path = os.path.join(relative_dir_path, "{}.{}.wav".format(hashed_text, speaker_id))
  ...
  if not os.path.exists(add_postfix(real_path, 0)):
    try:
      audio = synthesizer.synthesize(
        [text], paths=[real_path], speaker_ids=[speaker_id], attention_trim=True, isKorean=isKorean
      )[0]
...
def send_js(path):
  return send_from_directory(os.path.join(static_path, 'js'), path)
