...
def split_on_silence_with_librosa(
  ...
  for idx, (start, end) in enumerate(edges[skip_idx:]):
    ...
    output_path = "{}/{}.{:04d}.{}".format(os.path.dirname(audio_path), filename, idx, out_ext)
...
def split_on_silence_with_pydub(
  ...
  not_silence_ranges = silence.detect_nonsilent(audio, min_silence_len=silence_chunk_len, silence_thresh=silence_thresh)
