...
def get_continuous_audio_paths(paths, debug=False):
  ...
  if int(max_id) - int(min_id) + 1 != len(audio_ids):
    ...
    if debug:
      print("Missing audio : {} -> {}".format(paths, new_paths))
    return new_paths
  else:
    return paths
...
def replace_pred_with_book(
  ...
  threshold=0.9,
  max_candidate_num=5,
  min_post_char_check=2,
  max_post_char_check=7,
  max_n=5,
  max_allow_missing_when_matching=4,
  debug=False
):
    ...
    while True:
      ...
      for _pred in list(set([pred, koreanize_numbers(pred)])):
        ...
        for idx in range(min(max_skip, 10)):
          ...
          for jdx in range(min_post_char_check, max_post_char_check):
            sim = similarity("".join(_pred.split())[-jdx:], "".join(text.split())[-jdx:])
            end_sims.append(sim)

          candidate_candidates[text] = similarity(_pred, text)

      candidate, sim = get_argmax_key(candidate_candidates, with_value=True)
      ...
      if not prev_empty_skip:
        ...
        if 0 < search_idx < 4 and not prev_sentence_ended:
          ...
          if ends_with_punctuation(words_to_check[0]) == True:
            ...
            if debug:
              print(prev_key, tmp, new_preds[prev_key])
        ...
        if 0 < search_idx < 4 and prev_sentence_ended:
          ...
          if not any(ends_with_punctuation(word) for word in words_to_check):
            ...
            if debug:
              print(key, new_candidate, original_candidate)
      ...
      for id_idx in unfinished_ids:
        ...
        if pred in cur_ngram:
          ...
          if len(tmp_keys) == len(cur_ngram):
            if debug:
              print(n_allow_missing, tmp_keys, cur_ngram)

            tmp_keys = get_continuous_audio_paths(tmp_keys, debug)
            text_matches.append([[" ".join(cur_ngram)], tmp_keys])
  ...
  while True:
    ...
    while idx + jdx + 1 < len(preds):
      ...
      if not sent_idx:
        if debug:
          print(" [!] NOT FOUND: {}".format(tmp_pred))
        break
    ...
    new_keys = get_continuous_audio_paths(sum([keys[jdx] for jdx in idxes_to_merge], []))
...
def get_text_from_audio_batch(paths, multi_process=False):
  results = {}
  items = parallel_run(get_text_from_audio, paths, desc="get_text_from_audio_batch")
  for item in items:
    results.update(item)
  return results
  ...
  while True:
    try:
      ...
      config = types.RecognitionConfig(
        encoding=enums.RecognitionConfig.AudioEncoding.FLAC, sample_rate_hertz=16000, language_code='ko-KR'
      )
...
if __name__ == '__main__':
  ...
  book_ids = list(set([os.path.basename(path).split('.', 1)[0] for path in paths]))
  book_ids.sort()

  def get_finished_ids():
    finished_paths = glob(os.path.join(config.asset_dir, "speech-*.json"))
    finished_ids = list(
      set([os.path.basename(path).split('.', 1)[0].replace("speech-", "") for path in finished_paths])
    )
    finished_ids.sort()
    return finished_ids
  ...
  pbar = tqdm(book_ids_to_parse, "[1] google_speech", initial=len(finished_ids), total=len(book_ids))
  ...
  if False:
    match_paths = get_paths_by_pattern(config.asset_dir, 'clean-speech-*.json')
