...
class DataFeeder(threading.Thread):
  ...
  def __init__(self, coordinator, data_dirs, hparams, config, batches_per_group, data_type, batch_size):
    ...
    # Load metadata:
    self.path_dict = get_path_dict(data_dirs, self._hp, config, self.data_type, n_test=self.batch_size, rng=self.rng)
    ...
    if self.is_multi_speaker:
      self.inputs, self.input_lengths, self.loss_coeff, self.mel_targets, \
        self.linear_targets, self.speaker_id = queue.dequeue()
    else:
